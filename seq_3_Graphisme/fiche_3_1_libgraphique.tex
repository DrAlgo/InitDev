...
Dans cette fiche, vous allez réaliser des programmes graphiques en vous aidant sur la {\tt libgrapythk} (voir mode d'emploi).

\section*{Comment utiliser la {\tt libgrapythk}?}

\begin{action}
%%%%%%%%COPIE DU REPERTOIRE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Pour utiliser la {\tt libgrapythk} dans vos programmes: 
\begin{enumerate}
  \item Assurez-vous d'avoir, dans le même répertoire que votre programe, le fichier {\tt ligrapythk.py} qui vous est fourni sur Moodle.
  \item Dans votre programme, placez en tête de fichier la commande {\tt import libgrapythk}
\end{enumerate}
\end{action}
    
\begin{lecture}
{\bf {\tt libgrapythk} sur votre machine.} Pour que la {\tt libgrapythk} fonctionne sur votre machine, vous devez disposer de {\it python3} et avoir installé les modules {\tt python3-tk} et {\tt PIL}.
\end{lecture}

\begin{action}
%%%%%%EXEMPLE_1%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item {\bf Premier exemple.} Lisez le code source qui vous est fourni avec la {\tt libgrapythk} dans {\tt main.py}, puis exécutez ce script. Le programme se déroule en même temps dans une fenêtre graphique et dans le terminal. Essayez de bien comprendre le lien entre ce qui se déroule et le contenu de {\tt main.py}.

\item Lisez attentivement la première page de la notice d'utilisation de la {\tt libgrapythk} à la fin de ce document.\\
{\bf C'est votre manuel de référence pour la librairie graphique, vous devez toujours l'avoir sous la main.}
\end{action}
